...
<body class="nav-md">
    	...
    	<div class="">
        	<div class="page-title">
            	<div class="title_left">
                	<h3>결제 <small>목록</small></h3>
              	</div>
              	<div class="title_right" style="text-align: right;">
                	<form:form commandName="searchVO" action="${contextPath}/admin/payment/paymentList" method="get">
              			검색조건
		                        	...
		                        	<div class="card-box table-responsive">
		                    			<table id="datatable" class="table table-striped table-bordered" style="width:100%">
		                      				...
		                      				<tbody style="font-size: 0.7em;">
		                      					<c:forEach var="payment" items="${resultList}" varStatus="formNum">
			                        				<tr>
			                          					<td>
			                          						${payment.memberIdx}
			                          					</td>
							                          	...
							                          	<td>
							                          		${fn:substring(payment.paymentDate,0,11)}
							                          	</td>
							                          	<td>
							                          	...
							                          	<td>
							                          		${fn:substring(payment.paymentEnddate,0,11)}
							                          	</td>
								                        <td><button class="btn btn-secondary btn-sm" type="button" onclick="deleteCheck('${payment.paymentIdx}')">삭제하기</button></td>
			                        				</tr>		                      					
		                      					</c:forEach>
		                      				</tbody>
		                    			</table>
    ...
    <script>
	    ...
	    function link_page(pageNo){
    		...
    		document.getElementById("listForm").action = "<c:url value='/admin/payment/paymentDeleteList'/>";
    	   	document.getElementById("listForm").submit();
    	}
		
		function deleteCheck(paymentIdx){
			var res = confirm("정말 삭제하시겠습니까?");
			if(res){
	    		location.href='<c:url value="/admin/payment/paymentDelete?paymentIdx=' + paymentIdx + '&pageIdx=${searchVO.pageIndex}&searchCondition=${searchVO.searchCondition}&searchKeyword=${searchVO.searchKeyword}"/>';    			
			}
		}
    </script>
</body>
