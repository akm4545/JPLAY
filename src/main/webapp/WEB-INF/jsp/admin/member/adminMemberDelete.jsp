...
<body class="nav-md">
    	...
    	<div class="">
        	<div class="page-title">
            	<div class="title_left">
                	<h3>회원 <small>탈퇴</small></h3>
                	<input type="button" value="일괄삭제" class="btn btn-dark" onclick="totalDelete()"/>
              	</div>
              	<div class="title_right" style="text-align: right;">
                	<form:form commandName="searchVO" action="${contextPath}/admin/member/memberList" method="get">
              			검색조건
		                        	...
		                        	<div class="card-box table-responsive">
		                    			<table id="datatable" class="table table-striped table-bordered" style="width:100%">
		                      				...
		                      				<tbody style="font-size: 0.7em;">
		                      					<c:forEach var="member" items="${resultList}" varStatus="formNum">
			                        				<tr>
		                      							<td>
		                      								<input type="checkbox" value="${member.memberIdx}" name="total_delete">
		                      							</td>
			                          					<td>
			                          						${member.memberIdx}
			                          					</td>
							                          	...
							                          	<td>
							                          		${fn:substring(member.memberDeldate,0,11)}
							                          	</td>
								                        <td>
								                        	<button class="btn btn-secondary btn-sm" type="button" onclick="deleteMember('${member.memberIdx}')">
								                        		삭제하기
								                        	</button>
								                        </td>
			                        				</tr>		                      					
    ...
    <script>
    	...
    	function link_page(pageNo){
    		...
    		document.getElementById("listForm").action = "<c:url value='/admin/member/memberDeleteList'/>";
    	   	document.getElementById("listForm").submit();
    	}
    	
    	function deleteMember(idx){
    		location.href="<c:url value='/admin/member/memberDelete?memberIdx=" + idx + "&pageIndex=${searchVO.pageIndex}&searchCondition=${searchVO.searchCondition}&searchKeyword=${searchVO.searchKeyword}'/>";
    	}
    	
    	function totalDelete(){
    		var checkList = $('input:checkbox[name="total_delete"]:checked');
    		var length = $('input:checkbox[name="total_delete"]:checked').length;
    		var deleteList = "";
    		
    		if(length == 0){
    			alert("일괄삭제 항목을 체크하세요");
    			return;
    		}else{
	    		for(i=0; i<length; i++){
	    			deleteList += checkList[i].value + ","; 
	    		}
	    		var _deleteForm = document.createElement("form");
	    		var _deleteList = document.createElement("input");
	    		
	    		_deleteForm.method = "post";
	    		_deleteForm.action = "<c:url value='/admin/member/totalDelete'/>";
	    		
	    		_deleteList.name = "deleteList";
	    		_deleteList.type = "hidden";
	    		_deleteList.value = deleteList;
	    		
	    		_deleteForm.appendChild(_deleteList);
