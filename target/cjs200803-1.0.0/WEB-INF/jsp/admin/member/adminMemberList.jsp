		                      				...
		                      				<tbody style="font-size: 0.7em;">
		                      					<c:forEach var="member" items="${resultList}" varStatus="formNum">
			                        				<tr>
							                          	...
							                          	<td>
							                          		${fn:substring(member.memberJoindate,0,11)}
							                          	</td>
							                          	<td>
							                          		${member.memberDel}
							                          	</td>
								                        <td><button class="btn btn-secondary btn-sm" type="button" onclick="location.href='<c:url value="/admin/member/memberUpdateView?memberIdx=${member.memberIdx}&pageIndex=${searchVO.pageIndex}&searchCondition=${searchVO.searchCondition}&searchKeyword=${searchVO.searchKeyword}"/>';">수정하기</button></td>
			                        				</tr>		                      					
		                      					</c:forEach>
		                      				</tbody>
		                    			</table>
    ...
    <script>
    	$(document).ready(function(){
	    	...
	    	function link_page(pageNo){
	    		...
	    		document.getElementById("listForm").action = "<c:url value='/admin/member/memberList'/>";
